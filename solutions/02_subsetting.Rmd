...
1. (1) Use `filter(between())` to find characters whose heights are between 160 and 180 and (2) count the number of these observations.  
...
df %>%
  filter(heights >= 160 , heights <= 180) %>%
  nrow() # 21 

df %>%
  filter(between(heights, 160, 180)) %>%
  nrow() # 21
