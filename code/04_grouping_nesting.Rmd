...
# The Interquartile Range = The Difference Between 75th and 25th Percentiles 
...
- `broom::glance(model)`: for evaluating model quality and/or complexcity 
- `broom::tidy(model)`: for extracting each coefficient in the model (the estimates + its variability)
- `broom::augment(model, data)`: for getting extra values (residuals, and influence statisics)
